...
| **Moderation queue (admin)** | Admin > Moderation queue (`app/(admin)/moderation-queue.tsx`) and Reports (`reports.tsx`) | Same list across campuses. Admin actions: Official Warning, Purge and Take Down Content, 7-Day Account Shadowban, Permanent Account Termination. Actions that target a user call `suspend_user_account` (records the reason, writes an audit row). |
...
| **Moderation audit log** | Admin > Moderation audit log (`app/(admin)/moderation-audit-log.tsx`) | Filtered ledger: Reports (resolved/dismissed), Events, Verification, Communities, High-Risk (escrow release, impersonation). Each entry has actor, action, target, campus, reason and timestamp. |
