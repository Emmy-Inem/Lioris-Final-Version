...
CREATE TABLE IF NOT EXISTS posts (
    ...
    audience_scope TEXT DEFAULT 'global', -- 'global', 'student', 'alumni', 'staff'
    title TEXT,
...
-- Forum Communities (sub-forums). Posts are instant/no-approval; a proposed
-- COMMUNITY is what's held for root-admin review (see ForumsModerationTab) -
-- it stays invisible to everyone but its creator until approved, at which
-- point it becomes a real space anyone can post into.
CREATE TABLE IF NOT EXISTS forum_communities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    label TEXT NOT NULL,
    category TEXT NOT NULL, -- matches posts.category
...
-- must go through the separate admin-only UPDATE policy below to go live.
...
-- Seed the launch communities as pre-approved, system-owned spaces.
