...
export function LegacyVaultModalContent({ onReleased }: { onReleased: (amount: number) => void }) {
 ...
 <AppButton
 label="Force release to cold storage"variant="accent"disabled={!canConfirm}
...
export function ImpersonatorModalContent({ onStart }: { onStart: (targetUid: string, reason: string) => void }) {
 ...
 <AppTextField
 label="Reason (required, goes to audit log)"placeholder="e.g. Reproducing reported dashboard bug #482"value={reason}
 ...
 <AppButton
 label="Start shadowing"variant="accent"disabled={!canStart}
